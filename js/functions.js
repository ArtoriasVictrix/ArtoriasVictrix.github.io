...
$(window).on('load', function () {
  ...
  mobileMenu(); // Модальные окна

  function modal() {
    $('[data-modal-target]').on('click', function () {
      var target = $(this).attr('data-modal-target');
      $(target).addClass('_show').fadeIn();
    ...
    $('.modal button').on('click', function () {
      if ($(this).attr('data-modal-target')) {} else {
        $('body').removeClass('no-scroll');
        $('.modal__inner').removeClass('_open');
      }

      $(this).parents('.modal').removeClass('_show').fadeOut();
    });
    $(document).on('click', function (evt) {
      var target = $(evt.target);

      if ($(target).hasClass("modal") || $(target).hasClass("modal__container")) {
        $('.modal').removeClass('_show').fadeOut();
        $('body').removeClass('no-scroll');
        $('.modal__inner').removeClass('_open');
      }
    });
  ...
  modal(); // Прилипающее меню

  function scroll() {
    if ($(document).scrollTop() > 50) $('._primary').addClass('_fixed');else $('._primary').removeClass('_fixed');
  }
  ...
  $('input[type="tel"]').mask("+7 (999) 999-9999"); // Раскрытие изображений
  ...
  $.notify.addStyle("notification", {
    html: "<div>" + "<div class='title' data-notify-html='title'/>" + "<div class='text' data-notify-html='text'/>" + "</div>"
  ...
  $('.tabs__label').on('click', function () {
    $(this).addClass('_active').siblings().removeClass('_active').closest('.tabs').find('.tabs__content').removeClass('_active').eq($(this).index()).addClass('_active');
  }); // Счетчик корзины
  ...
  var actual,
      ...
      animating = false; // Добавление товара
  ...
  function basket(amount, count) {
    ...
    setTimeout(function () {
      cartCount.removeClass('active').removeClass('up down');
    }, 500);
    setTimeout(function () {
